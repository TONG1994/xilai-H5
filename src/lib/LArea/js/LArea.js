...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        init: function(params) {
            ...
            this.valueAddress=document.querySelector(params.valueAddress)
        ...
        finish: function(e) {
            ...
            if(_self.trigger.nodeName.toLowerCase() === 'input'){
                _self.trigger.value = provinceText + ((cityText)?(' ' + cityText):(''))+ ((countyText)?(' ' + countyText):(''));
            }else {
                _self.trigger.textContent = provinceText + ((cityText)?(' ' + cityText):(''))+ ((countyText)?(' ' + countyText):(''));
            }
            _self.value = [provinceVal, cityVal, countyVal];
            _self.valueAddress.value = [provinceVal, cityVal, countyVal];
            if(this.valueTo){
                this.valueTo.value= provinceCode +((cityCode)?(',' + cityCode):('')) + ((countyCode)?(',' + countyCode):(''));
            }
            _self.close(e);
            // 2018.10.14 确认后删除详细地址
