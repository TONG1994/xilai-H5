...
function getAddressType() {
    let check = Util.LocalStorageGetter("address_search_key");
    if (check === "2") {
...
// 获取之前录入的地址
function getInfor() {
    let addressPhone = Util.LocalStorageGetter('addressPhone');
    let addressName = Util.LocalStorageGetter('addressName');
    let addressDoorNumber = Util.LocalStorageGetter('addressDoorNumber');
    let addressCity = Util.LocalStorageGetter('addressCity');
    let valueAddress = Util.LocalStorageGetter('valueAddress');
    // 获取 输入提示地址
    let sentAddress = JSON.parse(Util.LocalStorageGetter('sentAddress'));
    let addressSelectDetail = Util.LocalStorageGetter('addressSelectDetail');
    ...
    if (sentAddress != '' && sentAddress != null && sentAddress != undefined) {
        let city = sentAddress.geocodes[0].city,
...
$("input[name='address-type']").click(function () {
    let checked = $("input[name='address-type']:checked").siblings(".iconfont"),
...
function positioning() {
    ...
    function onComplete(data) {
        ...
        if (addressArray == undefined) {
            ...
            console.log(data);
            let address_province_city_district;
            ...
            let address_detail = data.formattedAddress.split(`${addressArray.province}${addressArray.city}${addressArray.district}`);
...
loginBtn(Dom.saveDataBtn, function () {
    ...
    Dom.loadingWrap.show();
    let location_code = Dom.locationCoder.val();
    if (location_code !== "") {
        saveData(location_code, Dom.saveDataBtn)
    } else {
        let addressCity_text = Dom.addressCity.html(),
        ...
        if (addressCity_text !== '' && addressCity_text !== '请选择' && addressDetail !== "" && addressDetail !== '请选择') {
            getLocationCode(addressAll_text).then(function (data) {
                let location_code = data.geocodes[0].location;
...
function saveData(type, data, dom) {
    let addressName_text = Dom.addressName.val(),
...
$("#addressPhone").blur(function () {
    let phoneValue = Dom.addressPhone.val();
    let copyphone = clipboardNumber(phoneValue)
...
Dom.addressName.bind('change', function () {
    let nameValue = Dom.addressName.val();
...
Dom.addressDoorNumber.bind('input propertychange', function () {
    let addressDoorNumber = Dom.addressDoorNumber.val();
...
function addressType() {
    let addressType = Util.LocalStorageGetter('address_search_key');
    return addressType
}
...
function getAddressBookIndex(v1, v2, v3) {
    let province = 0, city = 0, area = 0, i, j, k
    let all = provinceCityRegions;
    for (i in all) {
        if (v1 === all[i].label) {
            province = i;
...
Dom.addressCity.bind('input propertychange', function () {
    let addressCity = Dom.addressCity.html();
    let valueAddress = Dom.valueAddress.val();
    let addressCityCode = Dom.value1.val();
