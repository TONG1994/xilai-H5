...
function getaddressData() {
    let address_data_sting = Util.LocalStorageGetter("edit_data"),
    ...
    $("input[name='address-type']").each(function (index) {
        let input_radio = $("input[name='address-type']");
...
loginBtn(Dom.saveBtn, function () {
    ...
    let address = Dom.addressCity.html().split(' ').join('') + Dom.addressDetail.val();
    if (address !== "") {
        getLocationCode(address).then(function (data) {
            let location = data.geocodes[0].location
...
function receive(data) {
    ...
    if (data == 1) {
        let data = Util.LocalStorageGetter('sentAddress')
        if (data != null && data != undefined && data != '') {
            data = JSON.parse(data)
            let address = data.geocodes[0].formatted_address;
