...
let t;
let countdown = Util.CookiesGetter("countdownTime") || 59;
...
dom.verificationInput.bind('input propertychange', function () {
    let verification = dom.verificationInput;
...
function sentVerification(jsonData) {
    // 发送验证码
    ajax("/xilaisender_s/user/createVerificationCode",
        "post",
        jsonData,
        "json",
        function (data) {
...
function userLogin(jsonData) {
    ...
    ajax("/xilaisender_s/login/userLogin",
        "post",
        jsonData,
        "json",
        function (data) {
            if (data.errCode === ERROR) {
                ...
                let phoneValue = authUser.userName;
                let sessionId = data.object.sessionId;
                let ssoToken = data.object.ssoToken;
                let staffInfo = data.object.staffInfo;
                let idCard = staffInfo.idCard;
                let perName = staffInfo.perName;
                ...
                setTimeout(function () {
                    ...
                    if (Util.LocalStorageGetter("mini-url") !== null) {
                        if (Util.LocalStorageGetter("mini-url") === "../selectRealName/selectRealName" && checkIdCard()) {
                            ...
                            let native_url = Util.LocalStorageGetter("native-url");
